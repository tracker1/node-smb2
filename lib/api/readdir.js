  ...
  SMB2Request('open', {path: path}, connection, function(err, file) {
    ...
    // SMB2 query directory
      ...
      if (err && err.code != 'STATUS_NO_MORE_FILES')
        ...
        goonQueryDir(file, connection, files, options, cb);
      //if(err) cb && cb(err);
...
function goonQueryDir(file, connection, files, options, cb) {
  ...
  SMB2Request('query_directory_goon', file, connection,
          ...
          SMB2Request('close', file, connection, function(err) {
            cb && cb(
              ...
              , currentFiles.filter(
                ...
                v.CreationTime = FileTime.toDate({low: low, high: high}).
                toISOString();

                buffer = v.LastAccessTime;
                var low = buffer.readUInt32LE(0);
                var high = buffer.readUInt32LE(4);
                v.LastAccessTime = FileTime.toDate({low: low, high: high}).
                toISOString();

                buffer = v.LastWriteTime;
                var low = buffer.readUInt32LE(0);
                var high = buffer.readUInt32LE(4);
                v.LastWriteTime = FileTime.toDate({low: low, high: high}).
                toISOString();

                buffer = v.ChangeTime;
                var low = buffer.readUInt32LE(0);
                var high = buffer.readUInt32LE(4);
                v.ChangeTime = FileTime.toDate({low: low, high: high}).
                toISOString();

                var isDirectory = v.FileAttributes & FILE_ATTRIBUTE_DIRECTORY
                  ? true
                  : false;
