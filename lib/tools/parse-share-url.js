...
module.exports = share => {
  ...
  const smbUrl = URL.parse(share);
  if (smbUrl.protocol === '') smbUrl = URL.parse(`smb:${opt.share}`);
  if (smbUrl.protocol !== 'smb:' && smbUrl.protocol !== 'cifs:') {
    throw new Error('Invalid share protocol');
  }

  if (!smbUrl.hostname) {
    throw new Error('Invalid share, use "\\host\share" or "smb://host/share"');
  }

  const shareName = smbUrl.pathname.split('/').filter(p => p).join('/');
  if ((/\//).test(shareName)) {
  ...
  if (smbUrl.auth) {
    const auth = smbUrl.href.match(/\/\/([^@]+)@/)[1].split(/\:/).map(v => decodeURIComponent(v));
    if (auth.length) {
