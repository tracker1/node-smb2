...
SMB2Connection.requireConnect = function(method){
  return function() {
    ...
    final = scheduleAutoClose(connection, final);
    args.push(final);
    ...
    connection.socket.on('lookup', function(err, address, family, host) {if (err) error_callback(err);});
    connection.socket.on('timeout', function() {error_callback('Timeout');});
    connection.socket.on('end', function() {error_callback('Ended');});
    connection.socket.on('close', function(error){if (error) error_callback(error);});
